...
function toggle_wireguard() {
  status_wireguard && \
     SUDO_ASKPASS=~/.config/wofi/wireguard/wireguard.sh sudo -A systemctl stop $SERVICE_NAME || \
     SUDO_ASKPASS=~/.config/wofi/wireguard/wireguard.sh sudo -A systemctl start $SERVICE_NAME
}
