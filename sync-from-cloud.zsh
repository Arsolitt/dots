...
run_restore() {
    ...
    local original_path=$(restic snapshots --tag "$tag" --latest 1 --json \
...
main() {
    ...
    read "confirmation?Вы уверены, что хотите продолжить? (Введите 'yes' для подтверждения): "
    ...
    run_restore omp,configs "$HOME/.omp/agent" || ((error_count++))
